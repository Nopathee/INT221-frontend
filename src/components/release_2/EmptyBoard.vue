...
import { getItems, addItem, deleteItemById, getItemById } from '@/libs/fetchUtils'
...
const saveTask = async (newTask) => {
  ...
  if (newTask.id === undefined) {
    const response = await addItem(`${import.meta.env.VITE_API_ENDPOINT}/v3/boards/${props.boardId}/tasks`, item)
...
const showDelete = async (taskToDelete, index) => {
  const task = await getItemById(`${import.meta.env.VITE_API_ENDPOINT}/v3/boards/${props.boardId}/tasks` , taskToDelete)
...
const confDelete = async () => {
  try {
    await deleteItemById(`${import.meta.env.VITE_API_ENDPOINT}/v3/boards/${props.boardId}/tasks`, deleteTask.value.item.id)
